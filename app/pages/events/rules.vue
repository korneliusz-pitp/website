...
<template>
  <UPage v-if="rules">
    <UContainer>
      <!-- Page Header -->
      <UPageHeader :title="rules.title" :description="rules.description" />
      ...
      <UPageBody>
          ...
          <UAccordion :items="accordionItems" type="multiple">
            <template #body="{ item }">
              <ProseUl v-if="getKeyRuleContent(Number(item.value)).length > 1">
                <ProseLi
                  v-for="(line, idx) in getKeyRuleContent(Number(item.value))"
                  :key="idx"
                >
                  <MDC :value="line" />
                </ProseLi>
        ...
        <UPageCTA
          ...
          :links="rules.callToAction.buttons ?? []"
        />
